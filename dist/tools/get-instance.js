...
export const getInstanceDescription = 'Get detailed information about a specific instance including connection details (IP address, SSH port, admin credentials). Secrets are encrypted and decrypted locally.';
function formatInstanceDetails(details) {
    const statusEmoji = {
        ordered: '📝',
        unprovisioned: '📝',
        provisioning: '🔄',
    ...
    if (details.secrets) {
        ...
        if (details.secrets.admin) {
            ...
            lines.push('');
            lines.push('```bash');
            lines.push(`ssh ${details.secrets.admin.username}@${details.secrets.ipv4} -p ${details.secrets.port}`);
            lines.push('```');
        }
    }
    else {
        lines.push('### Connection Details');
        lines.push('');
        lines.push('*Instance not yet provisioned. Connection details will be available once provisioning completes. Please retry in a few minutes*');
    }
    return lines.join('\n');
}
export async function getInstance(client, params) {
