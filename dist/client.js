...
export class X402Client {
    ...
    async createOrder(params) {
        ...
        if (amountUsd > this.maxPayment) {
            throw new Error(`Payment amount ${amountUsd} USDC exceeds maximum allowed ${this.maxPayment} USDC. ` +
     ...
     */
    async renewInstance(instanceId, period, type) {
     ...
     */
    async getInstanceDetails(instanceId, type) {
        const timestamp = Date.now().toString();
        const address = this.account.address;
     ...
     */
    async updateCustomName(instanceId, type, customName) {
        const timestamp = Date.now().toString();
        const address = this.account.address;
    ...
    parsePaymentRequirements(header) {
        try {
    ...
    async signPayment(requirements, resource) {
