...
**For the Claude Code plugin (remote server):**
The plugin does not read `~/.easy-node/.env`. You must set `EASYNODE_PRIVATE_KEY` as a system environment variable so Claude Code can pass it to the remote server.
